/* ============================================================
   樹木：標準樹木（tree.obj）／シマトネリコ／ソヨゴ／オリーブ
...
/* ============================================================
   標準樹木（tree.obj）
   ============================================================ */
const objBarkMat = new THREE.MeshLambertMaterial({ color: 0x5a4232, side: THREE.DoubleSide });
const objLeafMat = new THREE.MeshLambertMaterial({ color: 0x7ea94f, side: THREE.DoubleSide });
let treeRoot = null, treeState = 'idle', treeH = 3.0;

function loadTree(){
  if (treeState !== 'idle') return;
  treeState = 'loading';
  new OBJLoader().load('tree.obj', root => {
    /* 葉と幹をマテリアル名で分け、原点まわり・m 単位に直す */
    const b = new THREE.Box3().setFromObject(root);
    const s = b.getSize(new THREE.Vector3());
    const k = s.y > 100 ? 0.001 : 1;                     // mm なら m へ
    /* 葉と幹の振り分け（名前で分からなければ三角形が多い方を葉とする） */
    ...
    root.traverse(o => {
      ...
      byMat.set(nm, (byMat.get(nm) || 0) + triOf(o.geometry));
    });
    const biggest = [...byMat.entries()].sort((a, c) => c[1] - a[1])[0]?.[0] ?? '';
    const isBark = nm => /bark|trunk|wood|branch|stem|M_03/i.test(nm) ? true
                       : /leaf|leaves|foliage|M_02/i.test(nm) ? false : nm !== biggest;
    root.traverse(o => {
      if (!o.isMesh) return;
      const name = (Array.isArray(o.material) ? o.material[0] : o.material)?.name || '';
      o.material = isBark(name) ? objBarkMat : objLeafMat;
      o.geometry.translate(-(b.min.x + b.max.x)/2, -b.min.y, -(b.min.z + b.max.z)/2);
      o.geometry.scale(k, k, k);
      if (!o.geometry.attributes.normal) o.geometry.computeVertexNormals();
    });
    treeH = s.y * k;
    treeRoot = root; treeState = 'ready';
    rebuildAll();
  }, undefined, () => { treeState = 'error'; });
}

function buildObjTree(P){
  const g = new THREE.Group();
  if (treeState === 'idle') loadTree();
  if (treeState !== 'ready'){
    const c = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.14, P.height, 8),
      new THREE.MeshStandardMaterial({ color: 0x7b6a55, transparent: true, opacity: 0.5 }));
    c.position.y = P.height / 2;
    g.add(c);
    return g;
  }
  const inst = treeRoot.clone();
  inst.traverse(o => {
    if (!o.isMesh) return;
    o.castShadow = true; o.receiveShadow = true;
    o.userData.shared = true;
  });
  const k = P.height / treeH;
...
export default {
  ...
  items: [
    {
      id: 'tree_obj', name: '標準樹木（tree.obj）', place: 'point', rotatable: true,
      params: [{ k:'height', l:'樹高', min:1.5, max:8.0, step:0.1, def:3.0, fmt:v=>v.toFixed(1)+' m' }],
      build: buildObjTree,
      info: P => treeState === 'ready' ? `tree.obj ／ 樹高 ${P.height.toFixed(1)} m`
             : treeState === 'error' ? 'tree.obj を読み込めません' : 'tree.obj 読み込み中…',
    },
    ...Object.keys(SPECIES).map(name => ({
      ...
      options: [
        ...
        { k:'leafColor', l:'葉の色', type:'select',
          values:['標準','#4a7a3c','#7ea94f','#9dbd55','#8b9b76','#b5ac74'], def:'標準' },
      ],
      build: P => buildTree({ ...P, species: name,
        leafColor: P.leafColor === '標準' ? SPECIES[name].color : P.leafColor }),
