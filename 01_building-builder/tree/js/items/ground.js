...
function buildGrass(P, pl){
  ...
  const n = Math.min(MAX_BLADES, Math.round(w * d * P.density));
  if (n > 0){
...
function buildPavement(kind, P, pl){
  ...
  if (slit){
    ...
    const nBlade = Math.min(20000, Math.round(slitArea * 3000));
    if (nBlade > 0){
      const geo = bladeGeometry();
  ...
  /* 粒（骨材・砂利） */
  const n = Math.min(MAX_GRAINS, Math.round(w * d * P.gdens));
  if (n > 0){
...
const paveItem = (id, name, kind, extraOpts = []) => ({
  ...
  params: [
    ...
    { k:'soil',   l:'舗装厚', min:0.02, max:0.20, step:0.01, def:0.06, fmt:v=>Math.round(v*1000)+' mm' },
  ],
  options: extraOpts,
  ...
  items: [
    {
      ...
      params: [
        ...
        { k:'soil',      l:'下地の厚み', min:0, max:0.20, step:0.01, def:0.04, fmt:v=>Math.round(v*1000)+' mm' },
      ],
      options: [
